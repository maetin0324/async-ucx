...
use async_ucx::ucp::WorkerAddressInner;
...
async fn client(server_addr: String) {
    ...
    let context = async_ucx::ucp::Context::new().unwrap();
    let worker = context.create_worker().unwrap();
    let worker_clone = worker.clone();
    tokio::task::spawn_local(worker_clone.polling());
    ...
    // Use Vec instead of static array to avoid memory registration issues
    ...
    for _ in 0..N {
        ...
        endpoint
            .am_send(12, &HEADER, &data, true, Some(async_ucx::ucp::AmProto::Rndv))
            .await
            .unwrap();
        inflight += 1;
    }
...
async fn server() -> anyhow::Result<()> {
    ...
    // for i in 0u8.. {
        ...
        let jh = tokio::task::spawn_local(async move {
            ...
            let reply_header = vec![0u8; 1];
            let reply_data = vec![0u8; 1];
            for i in 0..(1 << 20) {
                ...
                unsafe {
                    msg.reply(16, &reply_header, &reply_data, false, None).await.unwrap();
                }
                // let stream = stream.clone();
